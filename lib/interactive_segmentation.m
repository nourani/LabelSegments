...
data.image.n_objects = max(max(seeds(:)),0);
...
fh_ui   = fh + 0; % figure for displaying the user interface
...
data.ui.obj_id_color = uicontrol(fh_ui, ...
   'Style',  'text', ...
   'String', '', ...
   'BackgroundColor', [0.8 0.8 0.8], ... get_obj_color(data, data.state.obj_id)
...
data.ui.image_store = uicontrol(fh_ui, ...
   'Style',    'pushbutton', ...
   'String',   'Store', ...
...
%    end
   ...
   function disp_update(data)
      ...
      % create image + boundary overlay
      img = boundary_overlay(dimg.im, labels);
      disp_update_ui(dimg.ucm, dimg.seeds, img, data.figures.fh);
      
      %disp_update_ui(dimg.ucm, dimg.seeds, dimg.im, data.figures.fh);
